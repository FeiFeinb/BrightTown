    ...
    public static class DialogueGraphExtensionMethod
    {
            ...
            DialogueGraphView graphView)
        {
            return basePorts.Select(basePort =>
                {
                    var connectedEdge = graphView.edges.ToList().Where(edge => edge.output == basePort || edge.input == basePort)
                        .ToList();
        ...
        public static void ClearEdgesAndNodes(this DialogueGraphView graphView)
        {
            ...
            // 清空节点
            foreach (Node selectViewPort in graphView.nodes.ToList())
            {
        ...
        public static void Save(this DialogueGraphSO _selectSO, DialogueGraphView graphView)
        {
            ...
            // 储存节点数据
            _selectSO.Clear();
            IEnumerable<DialogueGraphBaseNode> baseNodes = graphView.nodes.ToList().Cast<DialogueGraphBaseNode>();
            ...
            // 构建节点连线数组
            foreach (Edge edge in graphView.edges.ToList().Where(edge => edge.input != null))
            {
        ...
        public static void Load(this DialogueGraphSO _selectSO, DialogueGraphView graphView)
        {
            ...
            // 连线
            List<DialogueGraphBaseNode> baseNodes = graphView.nodes.ToList().Cast<DialogueGraphBaseNode>().ToList();
